...
/**
 * 进程执行引擎，包括使用javac、javap、java命令实现对项目的：
 * - 编译源代码操作
 * - 反编译.class文件操作
 * - 执行程序操作
...
public class ProcessExecuteEngine {
     ...
     * @return 最后一条命令启动的进程
     */
    public static Process runCommand(String... commands){
        try {
...
//                    }.start();
//
//                    //System.out.printf(String.valueOf(process.exitValue()));
...
//
    ...
    public static ProcessResult runCommand2(String... commands){
        try {
            Runtime runtime = Runtime.getRuntime();
            OS os = osType();   //不同操作系统执行的命令不同
            Process process = null;
            ...
            try {
                ...
                //System.out.printf(String.valueOf(exitCode));
                //if (os==OS.Windows) runCommand("cmd cd "+projectPath+" & del .list");
    ...
    public static ProcessResult buildProject(String projectPath){
        ...
        if(os == OS.Linux || os == OS.MacOS) {
            ...
            String dirStr = projectPath+"/out";

            new File(dirStr);


            process = runCommand("cmd cd "+projectPath+" & dir *.java/s/b > "+projectPath+"/.list",
                    "javac -s "+projectPath+" -d "+projectPath+"/out @"+projectPath+"/.list");

            System.out.println("cmd cd "+projectPath+" & dir *.java/s/b > "+projectPath+"/.list"+" javac -encoding utf-8 -s "+projectPath+" -d "+projectPath+"/out @"+projectPath+"/.list");
